...
- `bin/omarchy-kids-theme` (T45) — the kid-side picker: lists `theme_list_installed` and applies the choice through Omarchy's own `omarchy-theme-set`, run as the kid, so it writes their own theme and live-reloads their own shell. `--list`, `--set THEME` and `--dry-run` exist for tests; the level configs bind it on `Super+Ctrl+Shift+Space`, Omarchy's own theme chord (T45). A pre-reader grid tile for it is still open.
