...
| 4 | A4 | Face | One of the twelve `share/avatars/*.svg` animals (Q18), as a keyboard list **of their names** — `lib/tui.sh` has no separate grid widget, a list is exactly as keyboard-complete, and a `gum` list cannot render an SVG. The drawing itself is visible where it matters: the portal tile, the launcher and the exit modal after setup. Issue #18 asked for the portraits on this screen; that is a new picker (an image-capable chooser, or a block-art preview), not a fix to this one. |
