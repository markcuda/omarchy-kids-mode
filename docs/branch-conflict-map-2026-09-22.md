...
entries kept). **Merge that branch first**, before the rest, or the driver is not in the tree yet and
the conflicts come back. `PROGRESS.md` and `CHANGELOG.md` are edited, not just appended (19 of the 31
PROGRESS branches rewrite existing lines), so they still need a manual union.
