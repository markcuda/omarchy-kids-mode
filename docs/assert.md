...
`skip boot-locks:portal` and performs no boot access. With zero kids, assert checks `units`, `devices` and `relay-away`, prints
...
### Machine-level (once per run, only while at least one kid is provisioned — except `units`, `devices` and `relay-away`, which are checked even with zero kids; see below)
