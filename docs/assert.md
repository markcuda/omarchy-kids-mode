...
`skip boot-locks:portal` and performs no boot access. With zero kids, assert checks `units`, `devices`, `queue`, `relay-away`, `relay-tls` and `courier-conf`, prints
...
| `theme:<account>` | The account's own `.../current/theme.name` is absent (ok — nothing to enforce), a name in the offered set (`theme_list_installed`: Omarchy's themes plus the package's kid collection; a kid's in-set choice passes and is never reverted), or a planted non-regular file (a FIFO/symlink/directory) or a name outside the set (fails). **Verify-only since T44** (the theme is a kid preference). The read is `O_NOFOLLOW` + `O_NONBLOCK` + a regular-file check, so a planted FIFO cannot hang the assert | `lib/theme.sh`'s `theme_apply_for` applies the profile's `theme`, or the parent's current theme when the profile has no override — the same writer `omarchy-kids-conf set <kid> theme <name>` uses |
...
### Machine-level (once per run, only while at least one kid is provisioned — except `units`, `devices`, `queue`, `relay-away`, `relay-tls` and `courier-conf`, which are checked even with zero kids; see below)
...
| `units` | The package's units are enabled: `omarchy-kids-boot-login`, its cleanup unit and `omarchy-kids-assert` in `multi-user.target.wants`, `omarchy-kids-authd.socket` and `omarchy-kids-wifid.socket` in `sockets.target.wants`, `omarchy-kids-time.timer`, `omarchy-kids-ask-collect.timer`, `omarchy-kids-review.timer` and
`omarchy-kids-relay-courier.timer` in `timers.target.wants` (R-BOOT-3, R-SEC-2, R-WIFI-2 — issue #26 added the second socket; R-ASK-1..3 — issue #25 added the timer), the list itself shared with `bin/omarchy-kids-wizard`'s own Apply-time `enable` (then `start` of the sockets and timers) via `lib/kids.sh` (issue #46, T20). **Runs even with zero kids provisioned** — unlike every other lock in this table — since it's machine-level, not per-kid: a fresh install before the first kid, or right after `omarchy-kids-remove` disables these again, still needs them back so the *next* wizard run's A2 (`docs/authd.md`) and Apply both work; without the first the owner's stock autologin also wins every boot | `systemctl enable` of the whole list, then (on a live system, not under `--root`) `systemctl start` of the sockets and timers |
| `relay-away` | The one drop-in that may widen the relay's fence (R-NOTIFY-11.1, N-10): `/etc/systemd/system/omarchy-kids-relayd.service.d/away.conf` is absent (the relay is fenced to the LAN by the package's unit) or is root-owned `0644` and exactly the text `omarchy-kids-notify away tailnet` writes (`[Service]` + the LAN list + `100.64.0.0/10`); no other `*.conf` sits in that directory. The fence file itself (`/usr/lib/systemd/system/omarchy-kids-relayd.service`) is the package's and is never written (I-7) — a foreign drop-in is where a widened fence would live, so the lock FAILs and leaves it for a human rather than deleting it. Presence is the parent's consent: the fix rewrites a present `away.conf` and never creates or deletes one, then (live only) reloads and restarts the relay | rewrite `away.conf` through a temp file + `mv -f`, `chmod 0644`, then `systemctl daemon-reload` + `try-restart omarchy-kids-relayd.service`; never creates, never deletes, never touches a foreign `*.conf` |
| `decisions` | The kid's own copies of their decided requests (R-NOTIFY-6): `/var/lib/omarchy-kids/<kid>/decisions` is absent or is root `0750` in the kid's own primary group with every `*.json` root `0640` in that group, none a symlink, for each provisioned kid. The copy is what the ask overlay shows the kid; the queue record is the decision. Presence is `ask.py decide`'s and `collect`'s: the fix never creates, removes, rewrites, regenerates or reads a copy | `chmod`/`chown` of the directory and each copy; never creates, never deletes, never rewrites |
| `queue` | The request queue (R-NOTIFY-7): `/var/lib/omarchy-kids/queue` is absent or is root `omarchy-parents` `0750` with every `*.json` record root `omarchy-parents` `0640`, none a symlink. What a kid asked for and how it was decided is for root and the parent group (the relay's unit, the desktop notifier), not for every local account; a kid writes their draft into their own `0700` outbox and root's collect moves it here. Presence is `omarchy-kids-ask collect`'s and `apply-grant`'s: the fix never creates, removes, rewrites or reads a record. **Runs even with zero kids** | `chmod`/`chown` of the directory and each record; never creates, never deletes, never rewrites |
| `relay-tls` | The relay's TLS pair (R-NOTIFY-11.4): `/etc/omarchy-kids/relay` is absent or is root `omarchy-parents` `0750` with `cert.pem` root `omarchy-parents` `0644` and `key.pem` root `omarchy-parents` `0640` where present, no symlinks, and nothing else in the directory. `key.pem` is the private half of every paired device's pin (a reader can impersonate the relay); a foreign entry is where a loose copy would sit, so the lock FAILs and leaves it for a human rather than deleting it. Presence is `omarchy-kids-notify enable`'s: the fix never mints, removes or reads a key. **Runs even with zero kids** | `chmod`/`chown` of the directory and the two files; never creates, never deletes, never rewrites, never touches a foreign entry |
| `courier-conf` | The courier's mailbox file (R-NOTIFY-11.4): `/etc/omarchy-kids/courier.conf` is absent or is a root-owned `0600` regular file, not a symlink. It carries the parent's Gotify `token=` or ntfy `reply_topic=` — a credential to the parent's own server. Presence is `omarchy-kids-notify mailbox`'s: the fix never creates, removes or reads it; no group is checked at `0600`. **Runs even with zero kids** | `chmod 0600` and `chown root`; never creates, never deletes, never rewrites |
