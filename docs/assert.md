...
| `theme:<account>` | The account's own `.../current/theme.name`, when set, is a name in the offered set (`theme_list_installed`: Omarchy's themes plus the package's kid collection) — **verify-only since T44** (the theme is a kid preference): a kid's in-set choice passes and is never reverted; a name outside the set fails. "ok" (nothing to fix) if no theme is set at all — a box provisioned before issue #53, or a parent with no theme to copy at provision time | `lib/theme.sh`'s `theme_apply_for` re-applies the profile's default — the same writer `omarchy-kids-conf set <kid> theme <name>` uses |
