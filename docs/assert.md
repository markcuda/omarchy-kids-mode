...
`skip boot-locks:portal` and performs no boot access. With zero kids, assert checks `units`, `devices`, `relay-away`, `relay-tls` and `courier-conf`, prints
...
### Machine-level (once per run, only while at least one kid is provisioned — except `units`, `devices`, `relay-away`, `relay-tls` and `courier-conf`, which are checked even with zero kids; see below)
