...
or an already-decided record. The record also carries the displayed `kid`/`kind`/`what` (required) and, for a time ask,
`minutes` (optional); the ask path re-checks them against the queue record, so a relabelled
