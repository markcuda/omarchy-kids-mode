...
| 4 -- **superseded 2026-09-21** | Level 3 | No longer hidden: the two-modes amendment (row 1 of the 2026-09-21 table below) makes it the **Desktop** mode, and both pickers offer it -- `lib/wizard-advanced.sh:377` and `lib/panel-kid.sh:383` read `"3\|Desktop\|..."` beside `"1\|App grid\|..."`, with the old Level 2 retired (`docs/levels.md:5`). What this row called unverified was then checked on a real box in the 2026-09-21 Level 3 pass | Nothing left on this row. The L3 questions that remain live in the later table (row 6, the file-manager bind) and in `docs/levels.md`'s open items |
...
| --- | --- | --- | --- |
| 1 -- **decided 2026-09-21, implemented** | Collapse R-DESK-3/Appendix E to two kid modes (`grid`, `desktop`; band defaults 3-8 grid, 9+ desktop) | `docs/phase1/SPEC-AMENDMENT-two-kid-modes.md`: its own "Owner decisions (2026-09-21)" section answers all five closing questions (keep the numeric `level` key; 6-8 defaults to Grid; the with-browser Grid kid keeps the tile; labels "App grid"/"Desktop"; Stock parent-only), and the tree carries them -- `share/bands/bands.toml` is `level = 1` for 3-5/6-8 and `level = 3` for 9-12/13+, `lib/wizard-advanced.sh:377` and `lib/panel-kid.sh:383` offer both modes with Level 2 retired | Nothing to decide; the amendment is merged and live-checked. This is what supersedes row 4 above |
...
| 8 -- **settled 2026-09-26** | The bar widget's open-request badge and its "Open requests" row both ran the root-only `omarchy-kids-ask list` from the parent's unprivileged session, so neither could work | Both halves are resolved. The count is now published in root-written `status.json` (`open_requests`; SPEC.md R-BAR-3 as amended, pinned by `test/shell.d/status-publication-test.sh`), so the widget polls nothing; and `omarchy-kids-ask list` is no longer root-only -- `bin/omarchy-kids-ask:503` is `is_root \|\| in_parent_group`. Live as the parent on the try-omarchy VM, 2026-09-26: `omarchy-kids-ask list` and `omarchy-kids --requests` both answered `no open requests`, exit 0. (The `docs/loop-bar-requests-finding` this row cited does not exist; the amendment and that run are what settled it.) | Settled, option 1. The two action rows now open Omarchy's floating terminal, since what they show needs a tty: run bare from the bar, `omarchy-kids` printed `tui: no terminal to ask` and **exited 0**, a silent no-op |
