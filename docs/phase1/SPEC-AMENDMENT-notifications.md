...
- R-NOTIFY-15.5 **Tests.** `test/shell.d/relayd-test.sh` (or the Python test beside `lib/relay.py`) proves: an open record's row has exactly the six keys of R-NOTIFY-15.1; a decided record's row carries `state` and `decided_at` and carries `reply` only when the record's is a non-empty string; a decided record older than the window, or with a non-integer `decided_at`, is in neither list. The app's model test proves: a `recent` row with a bad `state` or `decided_at` is dropped, `reply` is cleaned and capped at 80, and the section renders no tappable widget.
