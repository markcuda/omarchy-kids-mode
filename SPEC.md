...
- R-NOTIFY-6 A kid learns the outcome through a root-written file under their own directory (**not built on this branch** — the ask overlay shows the result instead; Appendix H's file section says so). The kid session holds no credential to the relay and gains no new way to act.
