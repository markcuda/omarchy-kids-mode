...
- R-NOTIFY-10 Every parent-facing label states what is enforced and what is not: the fence ("home network only", or the CGNAT range when away-from-home is on), the relay's inability to decide, and the platform limit on background delivery.
- R-NOTIFY-11 `omarchy-kids-assert` re-asserts, on every update and every boot, the devices directory and conf modes, the units list (which now includes the courier timer), the away drop-in that widens the relay's fence (`relay-away`, R-NOTIFY-11.1, N-10), and the modes of the two secret-holding files — the relay's TLS pair (`relay-tls`: the directory `0750 root:omarchy-parents`, `cert.pem` `0644`, `key.pem` `0640`, nothing else in it) and the courier's mailbox (`courier-conf`: `0600 root`) (R-NOTIFY-11.4) — never the packaged unit that carries the fence itself. `omarchy-kids-check` has a row for the fence (`relay-fence`, verify-only, a reinstall rather than an assert), a row for the away drop-in, a row for the devices registry (R-NOTIFY-11.2) and rows for the two secret-holding files (R-NOTIFY-11.4).
