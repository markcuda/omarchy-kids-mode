...
- R-BAR-3 Reads `/run/omarchy-kids/status.json`, root-written, group `omarchy-parents` readable. It carries each live kid's minutes-left and paused state (R-NOTIFY-7), the open-request count (`open_requests`) and the open-review count (`reviews`). The badge still runs `omarchy-kids-ask list` for its own count (`docs/bar.md`); reading it from the document is N-1's remaining work. It also carries the pairing window's expiry (`pairing_open_until`), never the token, so the relay can keep itself up while a phone pairs (R-NOTIFY-2/5).
...
- R-NOTIFY-11 `omarchy-kids-assert` re-asserts the devices directory and conf modes and the units list (which now includes the courier timer) on every update. Re-asserting the relay's address fence and a `omarchy-kids-check` row for the relay/devices are not built on this branch.
