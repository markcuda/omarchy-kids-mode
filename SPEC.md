...
- R-NOTIFY-11 `omarchy-kids-assert` re-asserts, on every update and every boot, the devices directory and conf modes, the units list (which now includes the courier timer), and the away drop-in that widens the relay's fence (`relay-away`, R-NOTIFY-11.1, N-10) — never the packaged unit that carries the fence itself. `omarchy-kids-check` has a row for the fence (`relay-fence`, verify-only, a reinstall rather than an assert), a row for the away drop-in and a row for the devices registry (R-NOTIFY-11.2).
