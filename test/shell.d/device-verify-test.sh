...
rec11 = devices.write_pairing(pdir, "d12", "decide,act", now)
scopes, reason = devices.validate_pairing(pdir, "d12", "deadbeef", "Phone", p, p, now)
check(reason == "bad-proof", "a wrong pairing proof is refused")
proof12 = devices.pairing_proof(rec11["token"], "Phone", p, p)
scopes, reason = devices.validate_pairing(pdir, "d12", proof12, "Phone", p, p, now)
check(scopes == "decide,act" and reason == "ok", "a wrong proof leaves the record for a correct retry")
