...
**Fixed on topic branches awaiting merge:** `apps.show_missing` is enforced again; a portal box's
parent-only LUKS slot map no longer fails `boot:no-kid-luks-slots`; the plugins shelf no longer
shifts fields on a no-age entry; a non-time `ask` request keeps its `asked_at`; `session-start`
reads the manifest's fields without an empty-field shift; `omarchy-kids-data` renders a no-title row
without shifting its visit count, treats a malformed History as exit 2, and no longer lets a grant
truncate the kid's data screen; the panel's Apps screen says when an app isn't installed; the kid
toast is sized to its message and clears the launcher clock; the launcher's time-left line updates
again; `docs/levels.md`, `docs/ask.md`, `docs/wifi.md`, `docs/panel.md` and `docs/apps.md` match what
was live-tested. Packaging fixes: `fix/install-packaging`.

**Dogfood note:** the VM's packaged QML files are older than the branches; a live check must install
the file under test first.
...
network list and join flow, a band-3-5 launcher); keeping `docs/loop-report.md` current.
