...
`boot:no-kid-luks-slots`; the plugins shelf no longer shifts fields on a no-age entry; a non-time
`ask` request keeps its `asked_at`; `session-start` reads the manifest's fields without an
empty-field shift; `omarchy-kids-data` renders a no-title row without shifting its visit count,
treats a malformed History as exit 2, and no longer lets a grant truncate the kid's data screen;
the panel's Apps screen says when an app isn't installed; `docs/levels.md` matches the live Level 3
pass; the kid toast is sized to its message and clears the launcher clock; the launcher's
time-left line updates again; `docs/ask.md` and `docs/wifi.md` no longer claim their surfaces never
ran. Packaging fixes: `fix/install-packaging`.

**Dogfood note:** the VM's packaged QML files are older than the branches (its `timesup.qml` said
"Finishing in", its wifi picker lacked fixes); a live check must install the file under test first.
...
placement; the remaining unverified surfaces (the portal after a kid exits, the picker's non-empty
network list and join flow, the panel shelf screen, a band-3-5 launcher); keeping
